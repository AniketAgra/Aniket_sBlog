...
export default function ManagePosts() {
  ...
  const res = await fetch(`/api/admin/posts?${qs.toString()}`, { credentials: 'include' });
  ...
  return (
    <div>
      <SectionHeader title="Manage Posts" subtitle="Edit or remove your posts" />
          ...
          <table className="min-w-[680px] w-full text-sm">
            <thead className="bg-white/[0.03]">
              <tr className="text-left">
                <th className="px-4 py-3 font-medium text-gray-300">Post Title</th>
                ...
                <tr key={p._id} className={idx % 2 ? 'bg-white/[0.015]' : ''}>
                  <td className="px-4 py-4">
                    <div className="flex items-start gap-3">
                      {/* Thumbnail placeholder if needed in the future */}
                      <div>
                    ...
                    <div className="flex justify-end gap-1.5">
                      ...
                      {String(p.status || 'published') === 'draft' && (
                        <button
                          ...
                          title="Publish"
                          onClick={async () => {
                            const res = await fetch(`/api/admin/posts/${p._id}`, {
                              method: 'PATCH',
