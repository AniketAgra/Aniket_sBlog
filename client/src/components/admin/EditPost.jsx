...
export default function EditPost({ id }) {
  ...
  const submitWithStatus = async (nextStatus, e) => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(data?.error?.message || data?.message || 'Failed to update post');
      const successMsg = nextStatus === 'draft' ? 'Draft updated' : nextStatus === 'published' ? 'Post published' : 'Post updated';
  ...
  return (
    <div>
      <SectionHeader title="Edit Post" subtitle="Update content and republish" />
